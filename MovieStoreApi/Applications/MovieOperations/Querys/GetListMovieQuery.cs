...
namespace MovieStoreApi.Applications.MovieOperations.Querys
{
    public class GetListMovieQuery
    {
        ...
        {
            var movies = _dbContext.Movies.Include(x => x.Genre).Where(x => x.IsActive == true).OrderBy(x => x.ID).ToList<Movie>();
